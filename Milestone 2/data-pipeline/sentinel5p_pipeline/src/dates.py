...
Deliberately duplicated from sentinel2_pipeline/src/dates.py rather than
shared — this folder must stay independently cloneable with zero imports
from outside itself.
"""
...
def descending_date_steps(cfg: dict) -> Iterator[Tuple[str, str, str]]:
    ...
    `label` is the window's END date (YYYY_MM_DD). GEE filterDate end is
    exclusive, so the yielded end is cursor_end + 1 day.
    """
    t = cfg["temporal"]
