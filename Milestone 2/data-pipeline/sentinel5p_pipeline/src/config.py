...
def load_config(path: str = "config.yaml") -> dict:
    ...
    if mode == "alt_design":
        logger.warning(
            ...
            "This adds NO2, which was NEVER confirmed to have run in\n"
            "production. Every export from this run will be tagged\n"
            "ALT_DESIGN_UNVERIFIED. Do not treat this output as equivalent\n"
            "to the existing production data.\n" + "=" * 70
        )
    else:
        logger.info(
            "Running in 'confirmed' mode — matches verified production logic "
            "(AerosolIndex + CO only)."
        )
...
def _validate(cfg: dict) -> None:
    ...
    products = (cfg.get("sentinel5p") or {}).get("products")
    if not products:
        raise ValueError("config sentinel5p.products must be a non-empty list.")
...
def get_active_products(cfg: dict) -> list:
    """Returns the product list to actually fetch, filtered by logic_mode."""
    products = cfg["sentinel5p"]["products"]
    mode = cfg.get("logic_mode", "confirmed")
    if mode == "confirmed":
        return [p for p in products if p["name"] in ("AER_AI", "CO")]
    return products  # alt_design: all products, including NO2
