#!/usr/bin/env python3
"""
Sentinel-5P (TROPOMI) Standalone Fetch Pipeline — CLI entrypoint.
...
def main():
    parser = argparse.ArgumentParser(description="Standalone Sentinel-5P GEE -> GCS export pipeline")
    ...
    if args.logic and args.logic != cfg.get("logic_mode"):
        ...
        if args.logic == "alt_design":
            logging.getLogger("sentinel5p").warning(
                "CLI --logic alt_design override active — outputs tagged ALT_DESIGN_UNVERIFIED."
            )
        else:
            logging.getLogger("sentinel5p").info("CLI --logic confirmed override active.")
