...
def submit_export(image, label: str, cfg: dict) -> str:
    ...
    description = f"{tag}s2_{label}"
    exp = cfg["export"]
    prefix = f"{exp['gcs_prefix']}/raw/sentinel2/{description}"
    ...
    task = ee.batch.Export.image.toCloudStorage(
        ...
        region=get_aoi(cfg),
        scale=cfg["sentinel2"]["export_scale_m"],
    ...
    task.start()
    logger.info(
        f"[sentinel2:{mode}] Export task submitted: {description} -> "
...
def reconcile_progress(progress: dict) -> dict:
    ...
    for entry in progress.values():
        ...
        if task is None:
            # Task list is finite/recent; leave as submitted so we do not
            # blindly re-export, but log so the operator can investigate.
            logger.warning(
        ...
        # READY/RUNNING: leave as submitted
    if updated:
