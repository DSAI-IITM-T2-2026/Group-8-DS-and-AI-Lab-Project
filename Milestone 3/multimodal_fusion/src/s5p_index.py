"""Sentinel-5P monthly mosaic index (one GeoTIFF per year-month)."""
...
def list_s5p_mosaics(gcs_prefix: str) -> pd.DataFrame:
    ...
    if df.empty:
        raise RuntimeError(f"No S5P mosaics under {gcs_prefix}")
    logger.info("Indexed %d S5P monthly mosaics", len(df))
...
def pick_mosaic(tile_index: pd.DataFrame, year: int, month: int) -> dict | None:
