#!/usr/bin/env python3
"""Build balanced fusion dataset: MVP rows + optimized Sentinel-2 patches."""
...
from src.sample import assert_year_splits, assign_split, balanced_sample, load_mvp_frames
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--limit", type=int, default=None, help="Cap samples after sampling (smoke)")
    p.add_argument("--skip-patches", action="store_true", help="Write sampled manifest only")
    p.add_argument(
        "--download-tiles",
        action="store_true",
        help="Download each S2 tile locally before extracting (faster)",
    )
    p.add_argument(
        "--keep-tiles",
        action="store_true",
        help="Keep downloaded tiles (default: delete after each tile)",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    assert_year_splits(
        ...
        val_year=int(cfg["split"]["val_year"]),
        test_year=int(cfg["split"]["test_year"]),
    )
    logger.info("Pre-sample rows=%d pos=%d", len(df), int(df["y_fire"].sum()))
    if len(df) == 0:
        raise SystemExit(
            "No MVP rows after date/fire-season filters.\n"
            "Rebuild mvp_era5_dem: --start 2022-05-01 --end 2025-11-30 --fire-season"
        )
        ...
        for filename, group in tqdm(grouped, total=grouped.ngroups, desc="S2 tiles"):
            ...
            if args.download_tiles:
                try:
                    ...
                    logger.warning(
                        "Tile download failed %s: %s — falling back to /vsigs/", filename, exc
                    )
                    read_path = vsigs
            else:
    ...
    meta = {
        ...
        "split_years": {
            s: sorted(pd.to_datetime(g["label_date"]).dt.year.unique().tolist())
