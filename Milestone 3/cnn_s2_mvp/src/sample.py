...
# Tabular feature columns expected from MVP (subset used by CNN MLP)
TABULAR_FEATURES = [
    "t2m_mean",
    "t2m_max",
    "t2m_min",
...
def balanced_sample(
    ...
    for split_name, group in df.groupby("split"):
        ...
        if len(neg) and n_neg_target > 0:
            if hard_negative and "t2m_max" in neg.columns and "rh_min_7d" in neg.columns:
                ...
                # Sample with probability proportional to hardness
        ...
        if len(part) > cap:
            # Preserve all positives if possible
            if n_pos >= cap:
    ...
    if not parts:
        raise ValueError(
            "balanced_sample received an empty frame (no train/val/test rows). "
            "Rebuild mvp_era5_dem for 2018–2021 before running the CNN dataset builder."
        )
