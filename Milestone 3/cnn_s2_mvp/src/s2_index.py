...
# s2_2018_080000000000-0000000000.tif  → year=2018, month=08
_TILE_RE = re.compile(r"s2_(\d{4})_(\d{2})(\d+)-(\d+)\.tif$")

# Fixed 2×2 CA mosaic layout (confirmed from GCS tiles)
...
def resolve_month_for_date(
    ...
    """Forward-fill: most recent available (year, month) with year-month <= date's month."""
...
def offsets_for_point(lon: float, lat: float) -> tuple[int, int]:
    """Map lon/lat to the 2×2 mosaic tile offsets without opening rasters."""
...
def pick_tile_for_point(
    ...
    """Return tile row dict covering (lon, lat) for that month."""
    ...
    if len(match) == 0:
        # Fallback: open-bounds search (rare edge cases)
...
def resolve_read_path(tile: dict, tiles_dir: Path | None) -> str:
    """Prefer local cached tile if present, else /vsigs/ path."""
