# Wildfire Prediction Studio API (Milestone 6)

FastAPI implementation of [`frontend-backend-api-contract.md`](../../frontend-backend-api-contract.md),
...
| `GET /risk-map` | ⚠️ Wired, needs a model | Reads the real `final_processed/<date>_test.parquet` for the requested day (local cache or GCS) and scores it with a real loaded model. Without `WILDFIRE_MODEL_ARTIFACT` configured, returns `503 model_unavailable`. `mode=forecast_7d` returns `422` (the pipeline only supports a 1-day lead). |
...
The champion model (LightGBM classifier + XGBoost, calibrated, scored on
...
training notebook as `wildfire_model.joblib`. It is not checked into this
repository (multi-hundred-MB artifact). To enable real inference:

```bash
cp .env.example .env
# WILDFIRE_MODEL_ARTIFACT=/path/to/wildfire_model.joblib
```

**Known caveat:** that artifact pickles a custom `BlendedFireProbabilityModel`
class defined inline in the training notebook. If joblib can't resolve that
class at load time (because it isn't importable outside the notebook's
`__main__` namespace), loading will fail with a clear error surfaced through
`GET /health` and `GET /model/metadata` — it will not silently degrade to
fake predictions. Re-exporting the class into an importable module
alongside the artifact is the standard fix; that's a training-notebook
change out of scope for this API package.
...
- **No single probability `threshold`.** The champion architecture blends a
  classifier and a ranker and alerts on a daily top-K, not a fixed
  probability cutoff (see `daily_blend` in `champion_86_features.json` and
  `false_alerts_per_day_at_25` in `metrics_summary.json`). `threshold` is
  reported as `null` rather than an invented `0.5`.
  `riskClass` is instead computed from fixed, documented probability bands
  in `app/risk_service.py` (`_RISK_BANDS`) — a deployment-tunable heuristic,
  clearly separated from anything the model was trained on.
...
pip install -r ../daily_pipeline/requirements.txt   # for the pipeline modules this package imports
cp .env.example .env
