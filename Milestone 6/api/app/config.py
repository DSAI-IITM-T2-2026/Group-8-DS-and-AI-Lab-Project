...
- ``daily_pipeline`` config (bucket, grid, feature contract) is loaded from
  ``Milestone 6/daily_pipeline/utils/config.yaml`` via the pipeline's own
...
def milestone6_root() -> Path:
    return api_root().parent


def daily_pipeline_root() -> Path:
    return milestone6_root() / "daily_pipeline"
...
class Settings:
    ...
    def __init__(self) -> None:
        ...
        artifact = os.environ.get("WILDFIRE_MODEL_ARTIFACT", "").strip()
        self.model_artifact_path: Path | None = Path(artifact).expanduser() if artifact else None
