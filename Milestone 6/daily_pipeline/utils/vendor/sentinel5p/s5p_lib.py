#!/usr/bin/env python3
"""Sentinel-5P helpers. Year-wide submit only runs under __main__ (do not import for side effects)."""
...
# Defaults for optional year CLI; daily pipeline overrides bucket/prefix per call.
GEE_PROJECT_ID = "plated-mechanic-418917"
YEAR = 2019
GCS_BUCKET = "plated-mechanic-s5p-2016-2025"
...
def get_aoi():
    geo = json.loads(gzip.decompress(base64.b64decode(CALIFORNIA_GEOJSON_GZIP_B64)))
    return ee.FeatureCollection(geo["features"]).geometry(100)


# Back-compat name used by build_grid etc. — set after Initialize in callers.
AOI = None
...
def build_grid():
    ...
    cells = _ensure_aoi().coveringGrid(projection, GRID_RESOLUTION_M)
...
def reduce_tiles():
    ...
    return _ensure_aoi().coveringGrid(projection, REDUCE_TILE_M)
