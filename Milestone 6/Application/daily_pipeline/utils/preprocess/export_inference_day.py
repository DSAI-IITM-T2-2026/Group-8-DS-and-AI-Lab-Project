...
def _assert_inference_contract(
    ...
    *,
    ...
    numeric = pruned[feature_cols].apply(pd.to_numeric, errors="coerce")
    non_finite = ~np.isfinite(numeric.to_numpy(dtype=float))
    if non_finite.any():
        bad_columns = numeric.columns[non_finite.any(axis=0)].tolist()
        raise ValueError(
            "Champion features must be finite before export; "
            f"{int(non_finite.sum())} invalid values remain in {bad_columns[:5]}"
        )
...
def validate_champion_artifact(
    frame: pd.DataFrame,
    label_date: date,
    daily_cfg: dict,
) -> list[str]:
    """Validate a newly built or previously uploaded champion-day parquet.

    Returning the locked feature order lets callers reuse the exact same
    contract for an existing-object short circuit and for a fresh export.
    """
    ...
    if cell_ids.duplicated().any():
        raise ValueError("Champion artifact contains duplicate cell_id rows")

    subset_path = resolve_path(daily_cfg, "fire_region_csv")
    subset_mode = daily_cfg.get("preprocess", {}).get("cell_subset", "high_medium_fire")
    expected_cells = load_cell_subset(str(subset_path), mode=subset_mode)
    if expected_cells is not None:
        actual_cells = set(cell_ids)
        unexpected_cells = actual_cells - expected_cells
        if unexpected_cells:
            raise ValueError(
                "Champion artifact contains cells outside the configured supported set "
...
def export_champion_day(
    ...
    *,
    ...
    validate_champion_artifact(pruned, label_date, daily_cfg)
