...
The complete submission write-up is stored beside the Milestone 4 work log:
[Milestone 4 Report](../Milestone%204%20Report.md).
...
- **S5P 2021:** `s5p_2021_mode: ready` in `config.yaml`. Rebuild with `s5p_cell_cache` + `s5p_attach`. Use `placeholder` only if 2021 must be forced unavailable.
- **KNN Stage C (separate folder):** `python scripts/build_stage_c_knn.py --n-neighbors 5` → `outputs/m4_shared_cache/stage_c_knn/` (does not replace `stage_c/`).
